...
3. 浏览并选择 `E:\code-note\other\personal-knowledge` 文件夹
4. 点击「选择文件夹」
...
| 设置项 | 值 | 为什么 |
|--------|-----|-------|
| 附件文件夹路径 | `raw/assets` | 剪藏文章和下载图片时，附件自动存到 raw/assets |
...
**用途**：在浏览器中一键剪藏网页文章为 Markdown，直接存入 `raw/articles/`。
...
4. 点击「剪藏」
5. 文章以 Markdown 格式保存到 `raw/articles/` 目录
6. 告诉 Claude Code：「raw/articles/ 里有新文章，请处理」
...
```mermaid
graph TD
    A[浏览器看到好文章] --> B[Web Clipper 剪藏到 raw/articles/]
    B --> C[Claude Code 处理新资料]
    C --> D[LLM 更新 Wiki 页面]
    D --> E[Obsidian 实时查看更新]
    E --> F[图谱视图检查关联]
    F --> G[发现新问题]
...
1. **收集**：用 Web Clipper 剪藏文章 → 存到 `raw/articles/`
2. **处理**：在 Claude Code 中告诉 LLM 处理新资料 → LLM 自动更新 Wiki
3. **浏览**：在 Obsidian 中打开更新后的 Wiki 页面 → 阅读摘要、点击双链跳转
4. **探索**：用图谱视图查看知识网络 → 发现孤立节点或新关联
5. **提问**：回到 Claude Code 提问 → LLM 基于已有 Wiki 回答 → 有价值的洞察存入 Wiki
6. **循环**：重复以上步骤，知识库越来越丰富

...
Web 应用收到一个 HTTP 请求，必须依次经过：认证（是否登录）、鉴权（是否有权限）、限流（是否超出调用频次）、日志记录——最后才交给业务代码。如果把这四步全塞进一个方法，就变成 200 行的上帝方法；如果各自独立，又需要一个"指挥官"来硬编码处理顺序，每次调整都要修改核心逻辑。

责任链模式让每一步变成一个独立的"处理者节点"，节点之间形成链，前一个处理完（或决定拦截）再传给下一个。新增或调整步骤只需操作链的组装，不碰任何已有节点。Servlet Filter 和 Spring Security Filter Chain 都是这个思想的直接实现。
